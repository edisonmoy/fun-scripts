-- Shared schema for the recruiter-triage system. Applied idempotently by
-- BOTH the Python automation (gmail-recruiter-triage) and the Node
-- dashboard (recruiter-dashboard) on startup, so neither side depends on
-- the other having run first. Keep this file as the single source of
-- truth for the data model - if you change a column here, update both
-- consumers' queries in the same change.
