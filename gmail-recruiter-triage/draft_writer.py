...
def _build_prompt(classification, thread, preferences):
    return (
        ...
        "The style instructions above may contain a placeholder token like <name> "
        "(Edison's own template, not something to leave in the output). If so, "
        "replace it with the sender's actual first name, parsed from the 'From' "
        "header or their email signature below - e.g. 'From: Dan Bruck "
        "<dan@x.com>' becomes 'Dan'. If no plausible name can be determined, "
        "use a neutral greeting like 'Hi there' instead of leaving the "
        "placeholder token literally in the output.\n\n"
